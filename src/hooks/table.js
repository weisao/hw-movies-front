...
export default function table (context) {
  const { $forceUpdate } = context.root
  const defaultPaginationOptions = reactive({
  ...
  const selectedSubRowKeys = reactive({})
   ...
   * @returns {{children: *}}
   */
  function customRenderWithTitle (text) {
   ...
   * @returns {{children: *}}
   */
  function customRenderWithSearchTitle (text, searchValue) {
    const value = text === null || text === undefined || text === '' ? '' : text
    const placement = `${value}`.length > 5 ? 'topLeft' : 'top'
    return {
      children: (
   ...
   * @returns {*}
   */
  function customRenderUpdateLine (text, record, newKey) {
   ...
   */
  function customRenderWithDic (text, dic) {
    if (_.isObjectLike(dic)) {
      const value = dic[text] === undefined ? text : dic[text]
      const placement = `${value}`.length > 5 ? 'topLeft' : 'top'
      return {
        children: (
          <a-tooltip placement={placement} title={value} overlayClassName='scrolled-tooltip'>
      ...
      return customRenderWithTitle(text)
    }
  }
  /**
   * 表格渲染是否删除线显示，字典转化
   ...
   * @returns {{children: *}|*|{children: *}}
   */
  function customRenderUpdateLineDic (argumentsObj) {
   ...
   * @returns {*}
   */
  function handleRowTextFromMergeKey (tableData, index, mergeKey) {
    return _.reduce(
      mergeKey,
      (sum, n) => {
        return sum + _.get(tableData[index], n)
      },
      '',
    )
  }
   ...
   */
  function customRenderMergeRow (text, record, index, tableData, mergeKey, delKey, dic, icon, iconClick) {
    const mergeText = handleRowTextFromMergeKey(tableData, index, mergeKey)
    ...
    if (index !== 0 && handleRowTextFromMergeKey(tableData, index - 1, mergeKey) === mergeText) {
      sameRowCount = 0
    } else {
      for (let i = index + 1; i < tableData.length; i++) {
        if (mergeText === handleRowTextFromMergeKey(tableData, i, mergeKey)) {
          sameRowCount++
        } else {
          break
        }
    ...
    Object.assign(obj, customRenderUpdateLineDic({ text, record, delKey, dic, icon, iconClick }))
   ...
   */
  function onNestedSelectChange (selectedRowKey, selectedRows, subDataName) {
    // console.log(selectedRowKeys, selectedRows)
    // console.log('onNestedSelectChange')
    selectedRowKeys = selectedRowKey

    // 对勾选的父表格行遍历
    _.each(selectedRows, n => {
      // console.log(this.selectedSubRowKeys[n.key])
      // 子表格存在勾选又非全选则为半勾选状态，不对子表格重新勾选
      if (!selectedSubRowKeys[n.key].length) {
        selectedSubRowKeys[n.key] = _.map(n[subDataName], 'key')
      }
    })

    // 对未勾选的父表格行遍历，清空子表格勾选
    _.each(selectedSubRowKeys, (v, k) => {
      // console.log(v, k)
      if (selectedRowKeys.indexOf(k) < 0) {
        selectedSubRowKeys[k] = []
      }
    })
  }
  /**
   ...
   * @param parentKey
   */
  function onNestedSelectSubChange (selectedRowKey, parentKey) {
    // console.log('selectedRowKeys changed: ', selectedRowKeys)
    // console.log('onNestedSelectSubChange')
    if (selectedRowKey.length) {
      if (selectedRowKeys.indexOf(parentKey) < 0) {
        selectedRowKeys.push(parentKey)
      }
    } else {
      selectedRowKeys = _.remove(selectedRowKey, n => {
        return n !== parentKey
      })
    }

    selectedSubRowKeys[parentKey] = _.uniq(selectedRowKeys)
    $forceUpdate()
  }
  /**
   * 嵌套表格子表格切换全选
   ...
   * @param parentKey
   */
  function onNestedSelectSubAll (selected, selectedRows, parentKey) {
    // console.log('onNestedSelectSubAll')
    let selectedRowKeys = _.map(selectedRows, 'key')
    ...
    if (selected) {
      ...
      selectedRowKeys = _.remove(selectedRowKeys, n => {
        return n !== parentKey
      })
    }
    selectedSubRowKeys[parentKey] = [...selectedRowKeys]
   ...
   */
  function handleRadioCustomRow (record, index) {
    return {
      on: {
        click: event => {
   ...
   * @returns {{on: {click: on.click}}}
   */
  function handleCheckboxCustomRow (record, index) {
    return {
      on: {
        click: event => {
          if (selectedRowKeys.includes(record.key)) {
            _.remove(selectedRowKeys, n => n === record.key)
          } else {
   ...
   * @param mergeKey 作为判断合并依据的列名, 数组格式
   */
  function handleMergeRowSelection (tableRef, mergeKey) {
    ...
    if (tableData.length) {
      ...
      for (let i = 0; i < trSet.length; i++) {
        const mergeText = handleRowTextFromMergeKey(tableData, i, mergeKey)

        let sameRowCount = 1
        if (i !== 0 && handleRowTextFromMergeKey(tableData, i - 1, mergeKey) === mergeText) {
          sameRowCount = 0
        } else {
          for (let j = i + 1; j < tableData.length; j++) {
            if (mergeText === handleRowTextFromMergeKey(tableData, j, mergeKey)) {
              sameRowCount++
            } else {
              break
            }
   ...
   * @return {*}
   */
  function validateTableFields (tableFieldRules, data, callback) {
    const tableData = _.cloneDeep(data)
    const fieldRules = tableFieldRules
    for (let i = 0; i < tableData.length; i++) {
      fieldRules.forEach((n, key) => {
        for (const rule of n.rules) {
          if (rule.required) {
            // 判断必填
            if (!tableData[i][key] || !tableData[i][key].trim()) {
              // 校验
              message.warning(`第${i + 1}行${rule.message}`)
          ...
          if (rule.validator) {
            let resMsg = ''
            rule.validator(key, tableData[i][key], res => {
              resMsg = res
            })
            if (resMsg) {
              message.warning(`第${i + 1}行${(n.label ? n.label : key) + resMsg}`)
          ...
          if (rule.correlation) {
            let resMsg = ''
            rule.correlation(key, i, tableData[i], res => {
              resMsg = res
            }) // key index row callback
            if (resMsg) {
              message.warning(`第${i + 1}行${(n.label ? n.label : key) + resMsg}`)
              return callback(false, { index: i, msg: resMsg })
            }
  ...
  onMounted(() => {
    paginationOptions.value = _.cloneDeep(defaultPaginationOptions)
  })
  return {
    selectedRowKeys,
    ...
    tableLoading,
    paginationOptions,
    customRenderWithTitle,
    customRenderWithSearchTitle,
    customRenderUpdateLine,
    customRenderWithDic,
    customRenderUpdateLineDic,
    handleRowTextFromMergeKey,
    customRenderMergeRow,
    getDefaultPaginationData,
    onNestedSelectChange,
    onNestedSelectSubChange,
    onNestedSelectSubAll,
    onSelectChange,
    handleRadioCustomRow,
    handleCheckboxCustomRow,
    handleMergeRowSelection,
    validateTableFields,
  }
}
